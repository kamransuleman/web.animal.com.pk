        ...
        <div class="container">
            <div class="row">
                ...
                <div class="{{ $contentColSm }} page-content col-thin-left mb-4">
                    <div
                        class="category-list {{ config('settings.list.display_mode', 'make-grid') }}{{ $contentColSm == 'col-md-12' ? ' noSideBar' : '' }}">
                        <div class="tab-box">
                                        ...
                                        @foreach ($displayModes as $displayMode => $value)
                                            <span class="grid-view{{ $currDisplay == $displayMode ? ' active' : '' }}">
                                                ...
                                                @else
                                                    ...
                                                    @endphp
                                                    <a href="{!! $displayModeUrl !!}"><i
                                                            class="{{ data_get($value, 'icon') }}"></i></a>
                                                @endif
                                            </span>
                                        @endforeach
                                    @endif
                                            ...
                                            @if (isset($orderByOptions) && !empty($orderByOptions))
                                                @foreach ($orderByOptions as $option)
                                                    @if (data_get($option, 'condition'))
                                                        ...
                                                        @endphp
                                                        <li><a href="{!! $optionUrl !!}"
                                                                rel="nofollow">{{ data_get($option, 'label') }}</a></li>
                                                    @endif
                                                @endforeach
                                            @endif
                                        </ul>
                        ...
                        @if (request()->filled('q') && request()->get('q') != '' && data_get($count, '0') > 0)
                            <div class="tab-box save-search-bar text-center">
                                <a id="saveSearch" data-name="{!! request()->fullUrlWithoutQuery(['_token', 'location']) !!}"
                ...
                <div class="card-body text-center">
                    ...
                    @if (!auth()->check() && config('settings.single.guests_can_post_listings') != '1')
                        <a href="#quickLogin" class="btn btn-border btn-post btn-listing"
                            data-bs-toggle="modal">{{ t('start_now') }}</a>
                    @else
                        <a href="{{ \App\Helpers\UrlGen::addPost() }}"
                            class="btn btn-border btn-post btn-listing">{{ t('start_now') }}</a>
                    @endif
                </div>
            </div>
        </div>
