...
$mapCanBeShown = file_exists(config('larapen.core.maps.path') . config('country.icode') . '.svg') && data_get($sectionOptions, 'show_map') == '1';
        ...
        <div class="col-xl-12 page-content p-0">
            <div class="inner-box">
                ...
                <div class="row">
                            ...
                            @if (!empty($cities))
                                <div class="relative location-content">
                                    ...
                                    @if ($mapCanBeShown)
                                        <h2 class="title-3 pt-1 pb-3 px-3" style="white-space: nowrap;">
                                            <i
                                                class="fas fa-map-marker-alt"></i>&nbsp;{{ t('Choose a city or region') }}
                                        </h2>
                                    @endif
                                    <div class="col-xl-12 tab-inner">
                                        <div class="row">
                                            @foreach ($cities as $key => $items)
                                                <ul
                                                    class="cat-list {{ $ulCol }} mb-0 mb-xl-2 mb-lg-2 mb-md-2 {{ count($cities) == $key + 1 ? 'cat-list-border' : '' }}">
                                                    @foreach ($items as $k => $city)
                                                        <li>
                                                            @if (data_get($city, 'id') == 0)
                                                                <a href="#browseLocations" data-bs-toggle="modal"
                                                                    data-admin-code="0" data-city-id="0">
                                                                    {!! data_get($city, 'name') !!}
                                                                </a>
                                                            @else
                                    ...
                                    @if ($showListingBtn)
                                        @if (!auth()->check() && config('settings.single.guests_can_post_listings') != '1')
                                            <a class="btn btn-lg btn-listing" href="#quickLogin" data-bs-toggle="modal">
                                                <i class="far fa-plus-square"></i> {{ t('Sell') }}
                                            </a>
                                        @else
                    ...
                    @includeFirst([
                        config('larapen.core.customizedViewPath') . 'home.inc.locations.svgmap',
