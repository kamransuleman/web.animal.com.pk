        ...
        <div class="container">
                    ...
                    @if ($multiCountriesIsEnabled)
                        @if (!empty(config('country.icode')))
                            @if (file_exists(public_path() . '/images/flags/24/' . config('country.icode') . '.png'))
                                <button
                                    class="flag-menu country-flag d-md-none d-sm-block d-none btn btn-default float-end"
            ...
            <div class="navbar-collapse collapse" id="navbarsDefault">
                            ...
                            @if (!empty(config('country.icode')))
                                @if (file_exists(public_path() . '/images/flags/32/' . config('country.icode') . '.png'))
                                    <li class="flag-menu country-flag d-md-block d-sm-none d-none nav-item"
                                        ...
                                        @if ($multiCountriesIsEnabled)
                                            <a class="nav-link p-0" data-bs-toggle="modal"
                ...
                <ul class="nav navbar-nav ms-auto navbar-right">
                    ...
                    <li class="nav-item d-md-none d-sm-block d-block">
                        <a href="{{ \App\Helpers\UrlGen::register() }}" class="nav-link"><i class="far fa-user"></i>
                            {{ t('sign_up') }}</a>
                    </li>
                    ...
                    <li class="nav-item postadd">
                        @if (config('settings.single.guests_can_post_listings') != '1')
                            <a class="btn btn-block btn-border btn-post btn-listing" href="#quickLogin"
                                data-bs-toggle="modal">
                                <i class="far fa-plus-square"></i> {{ t('Sell') }}
                            </a>
                        @else
                            <a class="btn btn-block btn-border btn-post btn-listing"
